...
public:
	...
	typedef std::hash_map<std::string, OctreeSceneNode*> ChildNodeMap;
...
protected:
	...
	*/
	...
	virtual OctreeSceneNode* createChildImpl(const std::string& name);



public:
	OctreeSceneNode(const std::string& name);
	~OctreeSceneNode();
...
//	void _findVisibleObjects(Camera* cam,);*/
	...
	OctreeSceneNode* createChild(const std::string& name,const Vector3& translate = Vector3::ZERO, const Quaternion& rotate = Quaternion::IDENTITY);
	...
	OctreeSceneNode* getChild(const std::string& name) const;
	OctreeSceneNode* removeChild(unsigned short index);
	OctreeSceneNode* removeChild(OctreeSceneNode* child);
	OctreeSceneNode* removeChild(const std::string& name);
